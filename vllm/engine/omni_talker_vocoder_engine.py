...
class ForwardRequestOutput:
    ...
    def __repr__(self):
        return (f"ForwardRequestOutput(request_id={self.request_id}, prompt_token_ids={len(self.prompt_token_ids)}, "
                f"finished={self.finished},output_tokens={self.num_output_tokens},first_token_id={self.first_token_id}, "
                f"prompt_embeds={self.prompt_embeds.shape if self.prompt_embeds is not None else None})")
    @staticmethod
    def load(request_id: str, step: int, folder: str = "saved_forward_outputs") -> 'ForwardRequestOutput | None':
        """Load the request output from a file."""
        filename = f"{request_id}-{step}.pkl"
        ...
        tensor_filename = f"{request_id}-{step}_embeds.pt"
        tensor_filepath = os.path.join(folder, tensor_filename)
        prompt_embeds = torch.load(tensor_filepath, map_location='cpu')

        # Create and return the object
        output =  ForwardRequestOutput(
        ...
        logger.info("Loaded forward output of %s at step %s", output, step)

        return output

    def save(self, folder: str = "saved_forward_outputs") -> None:
        folder = "saved_forward_outputs"
        filename = f"{self.request_id}-{self.num_output_tokens}.pkl"
        ...
        # Save main data
        main_filepath = os.path.join(folder, filename)
        with open(main_filepath, 'wb') as f:
            pickle.dump(data_to_save, f)

        tensor_filename = f"{self.request_id}-{self.num_output_tokens}_embeds.pt"
        tensor_filepath = os.path.join(folder, tensor_filename)
        # Move tensor to CPU before saving
        torch.save(self.prompt_embeds.cpu(), tensor_filepath)

        print(f"Saved forward output of {self.request_id} at step {self.num_output_tokens} to {main_filepath}")
...
class SynchronizedGenerator(Generator[ForwardRequestOutput, None, None]):

    def __init__(
        self,
        generator: AsyncGenerator[ForwardRequestOutput, None],
...
class RequestOutputSynchronizedGenerator(Generator[RequestOutput, None, None]):
...
class _MQOmniCode2WavEngine:
    ...
    def __init__(
        ...
        self.code2wav_progress: dict[
            str,
            tuple[int, torch.Tensor]] = collections.defaultdict(lambda: (0, torch.empty(0)))
        ...
        while True:
            if self.code2wav_dynamic_batch:
                ...
                if len(chunks_list) == 0:
                    ...
                    logger.info(" ####### batch count: %d", len(chunks_list))
...
class _MQCode2WavEngineClient:
    ...
    def __init__(self, ipc_path: str, engine_pid: int):
        self.context = zmq.asyncio.Context()  # type: ignore -j
        self._errored_with: Optional[BaseException] = None

        # Send RPCGenerateRequest to the MQLLMEngine.
        self.input_socket: Socket = self.context.socket(zmq.PUSH)
        self.input_socket.connect(f"{ipc_path}{IPC_INPUT_EXT}")

        # Receive streams of RequestOutput from the MQLLMEngine.
        self.output_socket: Socket = self.context.socket(zmq.PULL)
        self.output_socket.connect(f"{ipc_path}{IPC_OUTPUT_EXT}")

        # IPC path for acking heartbeats.
        self.heartbeat_socket: Socket = self.context.socket(zmq.PULL)
...
class OmniTalkerVocoderEngine:
    ...
    def __init__(
        ...
        code2wav_batched_chunk: Optional[int] = None,
        ...
        self.thinker_outputs: dict[
            str,
            queue.Queue[tuple[str, str, ForwardRequestOutput]],
        ] = collections.defaultdict(queue.Queue)
        self.thinker_finished: Set[str] = set()

        # used later? -- CHECKME
        model_name = "Qwen/Qwen2.5-Omni-7B"
        self.thinker_config = get_config(model_name, trust_remote_code=False)

        # thinker tokenizer is also used by the talker
        ...
        for i, (talker_type,
            ...
            engine_args.processor = model_name
            ...
            self.cornserve_sidecar_ranks = engine_args.cornserve_sidecar_ranks
            if engine_args.cornserve_sidecar_ranks:
                self.sidecar_client = Sidecar(
                    SidecarConfig(
                        sidecar_rank=engine_args.cornserve_sidecar_ranks[0],
                        group=engine_args.cornserve_sidecar_ranks,
                        # TODO: make this use modelconfig
                        recv_tensor_shape=(-1, 3584),
                        recv_tensor_dtype=torch.bfloat16,
                    )
                )
                logger.info("CS: Connected to sidecar with ranks %s",
                            engine_args.cornserve_sidecar_ranks)
        ...
        self.output_queue: dict[str, queue.Queue[Union[RequestOutput, tuple[
            np.ndarray, int], None]]] = collections.defaultdict(queue.Queue)
    ...
    def _listen_thinker(
        ...
        voice_type: str,
        generator: AsyncGenerator[ForwardRequestOutput, None],
        thinker_outputs: queue.Queue[tuple[str, str, ForwardRequestOutput]],
    ):
        """Create a thread to get outputs from thinker and stream to the talker."""
        def _listen():
            last_output = None
            logger.info(
                "Received request %s from Thinker, start to stream to talker",
                request_id,
            )
            for output in SynchronizedGenerator(generator, self._loop):
                ...
                last_output = output
                logger.info("Thinker get output loop done with num_output_tokens %d",
                            output.num_output_tokens)

            if last_output.num_output_tokens > 1:
    ...
    def _listen_talker(
        ...
        def _listen():
            ...
            for output in RequestOutputSynchronizedGenerator(generator, self._loop):
    ...
    def _thinker_to_talker(
        ...
        request_id: str,
        voice_type: str,
        output: ForwardRequestOutput | None,
        talker_client: MQLLMEngineClient,
    ):
        ...
        if output == None:
            logger.info("Thinker request %s canceled", request_id)
            asyncio.run_coroutine_threadsafe(
        ...
        if output.num_output_tokens == 1 and output.finished:
        ...
        if output.num_output_tokens == 2:
            ...
            prompt_token_ids = copy.deepcopy(output.prompt_token_ids)
        ...
        *,
        ...
        async def sidecar_req_generator(request_id: str) -> AsyncGenerator[ForwardRequestOutput, None]:
            ...
            while True:
                ...
                if num_received == token_data["num_prefill_tokens"]:
                    base_chunk_id = chunk_id
                    break
                if num_received > token_data["num_prefill_tokens"]:
                    raise ValueError(f"Received more prefill tokens than expected: {num_received} > {token_data['num_prefill_tokens']}")

            chunk_id += 1
            fchunk = do_recv(request_id, chunk_id)
            prefill_chunks.append(fchunk)

            yield ForwardRequestOutput(
                request_id=request_id,
                prompt_embeds=torch.cat(prefill_chunks, dim=0),
                first_token_id=token_data["first_token_id"],
            ...
            chunk_id += 1
            prev_chunk = do_recv(request_id, chunk_id)
            while True:
                chunk_id += 1
                chunk: torch.Tensor | None = do_recv(request_id, chunk_id)
                if chunk is None:
                    yield ForwardRequestOutput(
                        request_id=request_id,
                        prompt_embeds=prev_chunk,
                        first_token_id=token_data["first_token_id"],
        ...
        async def req_generator(request_id: str) -> AsyncGenerator[ForwardRequestOutput, None]:
            logger.info("Request generator for request_id: %s", request_id)
            counter = 1
            while True:
                result = await asyncio.to_thread(ForwardRequestOutput.load, request_id, counter)
                if result is None:
                    break
                yield result
                counter += 1
        ...
        if self.cornserve_sidecar_ranks:
            generator = sidecar_req_generator(request_id)
        else:
            logger.warning("DEBUG only: using req_generator for request_id: %s", request_id)
            generator: AsyncGenerator[ForwardRequestOutput, None] = req_generator(request_id)
        ...
        if not talker_params:
            assert self.talker_generation_config
        ...
        if not talker_params.stop_token_ids:  # type: ignore -j
            talker_params.stop_token_ids = []  # type: ignore -j
        talker_params.stop_token_ids.extend([  # type: ignore -j
            talker_hf_config.tts_codec_start_token_id,
            talker_hf_config.tts_codec_end_token_id,
        ])

        # talker: chunked output to reduce overhead of zmq
        talker_params.chunked_return = True  # type: ignore -j
        talker_params.chunked_return_first_size = self.code2wav_future_cache_size // (  # type: ignore -j
            2 if self.code2wav_frequency == "50hz" else 4)
        talker_params.chunked_return_size = self.code2wav_chunk_size // (  # type: ignore -j
            2 if self.code2wav_frequency == "50hz" else 4)

        if params and params.max_tokens:  # type: ignore -j
            factor = 1 if self._ignore_code2wav(voice_type) else 40
            talker_params.max_tokens = min(talker_params.max_tokens,  # type: ignore -j
                                           params.max_tokens * factor)  # type: ignore -j
