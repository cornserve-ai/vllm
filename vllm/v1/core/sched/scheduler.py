...
class Scheduler(SchedulerInterface):
        ...
        if self.cornserve_config:
            self.sidecar_receiver = Sidecar(
                SidecarConfig(
                    ...
                    recv_tensor_shape=(-1, self.vllm_config.model_config.get_hidden_size()),
    ...
    def update_from_output(
        ...
        if self.cornserve_config:
            for req_id, indexes in scheduler_output.scheduled_encoder_inputs.items():
                req = self.requests[req_id]
                for index in indexes:
                    pos = req.mm_positions[index]
                    logger.info("CORNSERVE: mark data_id done %s", pos.data_id)
