...
from vllm.multimodal.inputs import (DataForward, MultiModalDataDict, MultiModalEncDecInputs,
                                    MultiModalInputs)
    ...
    for k, v in mm_data.items():
        ...
        if any(isinstance(d, DataForward) for d in data_list):
            if not all(isinstance(d, DataForward) for d in data_list):
                raise ValueError(
                    f"DataForward is not supported for mixed data types "
                    f"for modality {k}."
                )
            data_ids = [d.id for d in data_list]
            processor_data = [d.data for d in data_list]
...
class InputPreprocessor:
    ...
    def _process_multimodal(
        ...
        mm_inputs = mm_processor.apply(prompt, mm_data, mm_processor_kwargs,
                                  return_mm_hashes)
        if saved_mm_data_ids:
            for k, v in saved_mm_data_ids.items():
                placeholders = mm_inputs["mm_placeholders"][k]
